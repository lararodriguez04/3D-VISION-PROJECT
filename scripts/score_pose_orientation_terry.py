...
import argparse
import sys
from pathlib import Path          # moved to top level — was only imported inside __main__
...
def score_image(image_path: str, checkpoint: str, rotate: bool) -> None:
    ...
    # Prefer the checkpoint's recorded rotate setting; se3_rotate takes
    # precedence if present (written by the updated train_bradley_terry.py).
    ckpt_rotate = ckpt.get('se3_rotate', ckpt.get('rotate', rotate))
    ...
    if results.pose_world_landmarks is None:
        ...
        plt.tight_layout(); plt.savefig('score_result.png', dpi=150, bbox_inches='tight'); print('Saved → score_result.png')
        return
    ...
    plt.savefig('score_result.png', dpi=150, bbox_inches='tight'); print('Saved → score_result.png')
