...
Pipeline:
  ...
  3. Normalise each pose:  Y-flip  →  centre at hip  →  scale by torso
                           →  full SE(3) body-frame alignment (Gram-Schmidt)
                           →  reflection canonicalisation (right shoulder → +X)
...
def _gram_schmidt_body_frame(xyz: np.ndarray) -> np.ndarray:
    """
    Build a 3×3 rotation matrix R that maps the pose into a canonical body frame
    using corrected Gram-Schmidt orthonormalisation on three anatomical vectors.

    Canonical axes after rotation
    ──────────────────────────────
      +Y  spine direction  (hip midpoint → shoulder midpoint)
      +X  right-shoulder direction  (after orthogonalising against spine)
      +Z  spine × right  (points forward out of the frontal plane)
    ...
    Parameters
    ----------
    xyz : (33, 3) float32  — already centred at hip midpoint, torso-scaled

    Returns
    -------
    R : (3, 3) float32  — rotation matrix;  xyz_canonical = (R @ xyz.T).T
    """
    shoulder_mid = (xyz[11] + xyz[12]) / 2.0   # hip midpoint is already origin

    # ── axis-1 : spine (hip → shoulder), normalised ──────────────────
    e_spine = shoulder_mid.copy()
    spine_len = float(np.linalg.norm(e_spine))
    if spine_len > 1e-6:
        e_spine /= spine_len
    else:
        e_spine = np.array([0.0, 1.0, 0.0], dtype=np.float32)

    # ── axis-2 : right-shoulder direction, orthogonalised against spine
    # (right shoulder = landmark 12; left = landmark 11)
    right_dir = xyz[12] - xyz[11]              # points left→right in body frame
    right_dir -= np.dot(right_dir, e_spine) * e_spine   # remove spine component
    right_len = float(np.linalg.norm(right_dir))
    if right_len > 1e-6:
        right_dir /= right_len
    else:
        # Degenerate (e.g. T-pose detected edge case): fall back to world X
        right_dir = np.array([1.0, 0.0, 0.0], dtype=np.float32)
        right_dir -= np.dot(right_dir, e_spine) * e_spine
        right_dir /= (np.linalg.norm(right_dir) + 1e-9)

    # ── axis-3 : forward = right × spine (right-hand rule → det=+1) ──
    forward = np.cross(right_dir, e_spine)
    forward_len = float(np.linalg.norm(forward))
    if forward_len > 1e-6:
        forward /= forward_len

    # Rows of R are the target canonical axes expressed in the input frame.
    # R maps input coords → canonical coords: x_canon = R @ x_input
    R = np.stack([right_dir, e_spine, forward], axis=0).astype(np.float32)
    return R
...
def normalize_pose(raw: np.ndarray, rotate: bool = True) -> np.ndarray:
    ...
    4. Full SE(3) body-frame rotation via corrected Gram-Schmidt (when rotate=True):
         +Y ← spine,  +X ← right shoulder,  +Z ← spine × right
       This removes yaw, pitch *and* roll — fixing the out-of-plane tilt that the
       old XY-only rotation left unhandled.
    5. Reflection canonicalisation: if the right shoulder ends up on the −X side
       (camera mirroring or ambiguous orientation), flip X so it is always on +X.
       Ensures mirror-image poses share the same representation.
    ...
    if rotate:
        # 4. Full SE(3) body-frame alignment (Gram-Schmidt, all 3 axes)
        R = _gram_schmidt_body_frame(xyz)
        xyz = (R @ xyz.T).T                          # (33, 3)

        # 5. Reflection canonicalisation
        #    After step 4, landmark 12 (right shoulder) should be on +X.
        #    If it is not, the pose was captured mirrored — flip X to canonicalise.
        if xyz[12, 0] < 0.0:
            xyz[:, 0] *= -1.0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                'val_acc':     val_acc,
                'epoch':       epoch,
                'rotate':      rotate,   # kept for back-compat with score_pose.py
                'se3_rotate':  rotate,   # explicit: True = full Gram-Schmidt + reflection
                'hidden':      [256, 128, 64],
                'input_dim':   132,
            }, CHECKPOINT)
